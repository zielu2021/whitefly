...
@app.route('/flaskasgi/form', methods=['GET', 'POST'])
def form():
    if request.method == 'POST':
        ...
        db.session.commit()
        return '''Data saved! <a href="/flaskasgi">Back to index</a>'''
    return render_template('form.html')

@app.route('/flaskasgi/async-form-page')
def async_form_page():
    return render_template('async_form.html')
"""This route serves the HTML template that includes the asynchronous form."""

@app.route('/flaskasgi/async-form', methods=['POST'])
def async_form():
